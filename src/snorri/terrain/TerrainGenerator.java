...
public class TerrainGenerator {
	...
	public enum Biome {
		...
		Biome(double ceiling, Tile tile) {
		...
		for (int x = 0; x < dim.getX(); x++) {
			for (int y = 0; y < dim.getY(); y++) {
				for (int i = 0; i < biomes.length; i++) {
					if (heightMap[x][y] < biomes[i].getCeiling()) {
						level.setTileGrid(x, y, biomes[i].getNewTile());
						break;
					}
				}
			}
	...
	protected double noise(Vector v) {
	...
	// 2D simplex noise
	protected static double noise(double xin, double yin) {
	...
	// 3D simplex noise
	protected static double noise(double xin, double yin, double zin) {
	...
	protected static double noise(double x, double y, double z, double w) {
